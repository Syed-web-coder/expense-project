...
### (e) Dockerfile uses `uv sync --no-dev` (no `--frozen`) for cross-platform portability

`uv.lock` stores the `expense-ai` path dependency as an absolute Windows path (`directory = "C:/Users/Admin/expense-project/expense-ai"`). uv only makes path dependency sources relative when the packages are members of a uv workspace; standalone path dependencies (`expense-ai @ {root:uri}/../expense-ai` in `pyproject.toml`) are always resolved to absolute paths at lock time. Using `[tool.uv.sources]` with `path = "../expense-ai"` produces the same absolute-path result. Consequently `uv sync --frozen` fails in the Linux container because the Windows path is unreachable. The Dockerfile drops `--frozen` and uses `uv sync --no-dev` instead, letting uv re-resolve from `pyproject.toml`'s version constraints at image-build time. Trade-off: the container may install newer patch versions than the developer's local environment, rather than the exact pinned set in `uv.lock`. The `--no-dev` flag is retained so test/lint tooling (`pytest`, `ruff`, `mypy`) is excluded from the runtime image.
