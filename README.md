...
GET /merchants/mer_synth_001 — accepts 200 or 404 (item may or may not exist)
GET /merchants/ — asserts 404 (no route)
Polls /aws/lambda/$FUNCTION_NAME CloudWatch logs for a recent REPORT line (up to 4 × 5 s retries)
...
sam delete removes the CloudFormation stack, Lambda function, API Gateway, DynamoDB table, log group, and alarm. The S3 bucket created by --resolve-s3 is not deleted automatically; remove it via the console or aws s3 rb s3://<bucket> --force if needed.containerd state desynced, requiring a full cluster teardown/recreate to recover. This is a documented, one-off environmental hiccup — not a manifest or CI issue — but explains the unusually high revision count in `kubectl rollout history` from this session.
