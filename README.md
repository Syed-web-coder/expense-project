...
## Week 7 Day 2
Pandas corpus loader (`corpus.py`) with batched MiniLM embeddings and an explicit `float32` boundary cast; `doc_chunks` pgvector schema (`vector(384)`, HNSW cosine index `m=16/ef_construction=64`) with an idempotent `ON CONFLICT (doc_id, chunk_idx, model_version) DO UPDATE` loader; LangSmith `@traceable` `retrieve_chunks` function verified live against project `expense-ai-dev`; 50-row RAGAS golden set (`tests/golden/expense_golden_50.jsonl`) with a `pytest.mark.skipif` guard (baseline pending Anthropic key); Great Expectations 1.x `doc_chunks_v1` suite (five expectations — not-null, row count, chunk-text length — run via ephemeral Testcontainers Postgres); and three new CI gates (GX checkpoint, RAGAS thresholds, LangSmith visibility — fork-safe via `$GITHUB_ENV` flag). 56 tests pass, 1 skipped. See [`expense-ai/`](expense-ai/).

## Week 7 Day 3
Full RAG pipeline assembled end-to-end in `expense-ai/`: schema V002 adds `chunk_metadata jsonb`, `content_hash`, `chunk_tsv tsvector GENERATED ALWAYS AS` plus per-tenant partial HNSW indexes (m=24, ef_construction=128) and GIN indexes; `chunker.py` (RecursiveCharacterTextSplitter, 900/150 defaults, chunk_id stamping); `hybrid.py` (dense + sparse FTS via `websearch_to_tsquery`, rank-based RRF fusion — no score normalisation); `rerank.py` (greedy MMR with injectable embedder, BGE CrossEncoder with `ThreadPoolExecutor` timeout-and-fallback and a `RERANK_TIMEOUT_COUNT` circuit-breaker counter); `cache.py` (Redis epoch-keyed semantic cache, `sha256(round(vec*100).int32)` bucket key, cross-tenant citation defence-in-depth); `rag.retrieve_and_generate` wiring the full pipeline with injectable anthropic/scorer/embedder clients; and a POSIX-only Airflow 3.x TaskFlow DAG (`expense_ai_ingest`, 5 tasks). Four new test files cover semantic cache (Testcontainers Redis), tenant isolation (DB-side assertion), RAGAS gate (faithfulness hard-fail / others `warnings.warn`), and a retrieve_and_generate integration test that verifies a cache hit on the second call (`fake_anthropic.call_count == 1`). 76 tests pass, 3 skipped (RAGAS×2 + DAG on Windows). See [`expense-ai/`](expense-ai/).


## Week 7 Day 5
LangGraph multi-agent capstone [`expense-agent-svc/`](expense-agent-svc/) built on top of `expense-ai` and `expense-mcp-server`. Three-node graph (`retrieval_agent`, `api_agent`, `synthesis_agent`) compiled with `AsyncPostgresSaver` for crash-resume persistence; per-request `BudgetGuard` (integer 1e-5 USD arithmetic, no floats); `@deadline` wrapping each node; SSE streaming endpoint (`POST /v1/chat/stream`) emitting `0:`/`2:`/`3:` frames; 20-scenario trajectory eval gate (`trajectory_match >= 0.70`); Argo CD `Application` manifest; AWS Budgets + `BudgetsAction` CloudFormation stack (DenyLlmProxyInvoke at 100%); paths-filtered GitHub Actions CI (PR: ruff → mypy → pytest → eval gate; merge-to-main: ECR + config-bump stubs). 28 tests pass (15 from W7 D5 Tasks 1+2, 13 new). Key deviation: injectable clients go in `config["configurable"]`, not `AgentState`, because `PostgresSaver` serialises state to JSON. See [`expense-agent-svc/README.md`](expense-agent-svc/README.md).
